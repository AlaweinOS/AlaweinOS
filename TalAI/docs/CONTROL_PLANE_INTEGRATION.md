...
+---------------------------+      |  - SOP_Validation         |
      ...
      v                                     v
+---------------------------+      +---------------------------+
| LLM Providers             |      | Optilibria               |
|  - Claude / OpenAI / ... |      |  - optimize(...)         |
+---------------------------+      |  - adapters (QAP, etc.)  |
                                   +---------------------------+
```

- **ATLAS Orchestrator**: LLM routing + budget enforcement + provider adapters.
- **ATLAS Autonomous Research**: research protocol orchestrator, built on top of the Orchestrator and optimization engines.
- **Turingo**: multi‑paradigm optimization pipeline that can use Optilibria as its classical/engine layer.
- **Optilibria**: universal optimization framework with method registry and domain adapters.

Each layer can be evolved independently as long as it preserves its interfaces.

---

## 2. ATLAS Orchestrator Control Plane

### 2.1 Role

ATLAS Orchestrator is the **only place** that should talk to raw LLM providers.
...
For higher‑level systems (ATLAS Research, Turingo, other tools), define a small wrapper:
...
Central configuration lives in ATLAS Orchestrator (code and/or config files):
...
## 3. ATLAS Autonomous Research Control Plane

### 3.1 Role

ATLAS Autonomous Research orchestrates a **full research pipeline**:
...
It should treat ATLAS Orchestrator as its **LLM substrate** and Turingo/Optilibria as **optimization substrates** when needed.

### 3.2 LLM integration

Where ATLAS Research uses LLMs (examples):
...
from .llm_client import llm_call  # thin wrapper around ATLAS Orchestrator
...
When hypotheses involve optimization methods (especially QAP or related problems), ATLAS Research can:
...
- Or call **Optilibria** directly for simple method evaluations:

  ```python
  from optilibria import optimize_qap
...
ATLAS Research’s meta‑learning system (UCB1 bandit) chooses personality agents for tasks like risk assessment and refutation.
...
It sits **on top of** optimization engines such as Optilibria.
...
- **Classical**: use Optilibria for supported problem types (e.g., QAP).
...
- Which Optilibria methods are valid per problem type.

This lets ATLAS Research (or other callers) ask Turingo for **capability reports**, e.g.:
...
## 5. Optilibria Control Plane

### 5.1 Role

Optilibria provides a **universal optimization interface** with:
...
This makes downstream consumers (Turingo, ATLAS Research) more robust and allows better benchmarking.
...
- Use ATLAS Research’s risk/refutation/interrogation stages to avoid accepting poorly supported hypotheses.
