"""
UARO-ATLAS Integration

Connects Universal Autonomous Reasoning Orchestrator (UARO) with
ATLAS Autonomous Research System.

This integration enables:
1. ATLAS research tasks → UARO Problems (automatic formulation)
2. ATLAS workflows → UARO Primitives (automatic extraction)
3. UARO solutions → ATLAS research outputs (with proof documents)
4. Bidirectional learning (ATLAS learns problem-solving, UARO learns domain knowledge)
"""
...
class ResearchProblem(Problem):
    """
    Adapter that converts ATLAS research tasks into UARO Problems
    ...
    def __init__(self, task: ResearchTask):
        """
        Initialize research problem from ATLAS task

        Args:
            task: ATLAS research task to convert
        """
        self.task = task
        self.current_state = {
            'phase': 'planning',
...
class WorkflowExtractor:
    """
    Extracts successful ATLAS workflows as reusable UARO primitives

    Learns from ATLAS's domain expertise by converting proven workflows
    ...
    def extract_from_solution(
        ...
        class ExtractedPrimitive(ReasoningPrimitive):
            """Primitive extracted from ATLAS workflow"""
...
class ATLASUAROIntegration:
    """
    Main integration class connecting ATLAS and UARO

    Provides:
    - Automatic problem formulation (ATLAS tasks → UARO problems)
    - Workflow extraction (ATLAS workflows → UARO primitives)
    - Explainable research (UARO proofs for ATLAS outputs)
    ...
    def __init__(
        ...
        """
        Initialize ATLAS-UARO integration
    ...
    def solve_research_task(
        ...
        """
        Solve ATLAS research task using UARO
    ...
    def publish_learned_workflow(
        ...
        listing_id = self.marketplace.publish(
            ...
            tags=tags or ['atlas', 'research', 'workflow']
        )
...
def create_atlas_uaro_integration() -> ATLASUAROIntegration:
    """Create new ATLAS-UARO integration instance"""
...
def solve_atlas_task(
    ...
    """
    Solve ATLAS research task (convenience function)
