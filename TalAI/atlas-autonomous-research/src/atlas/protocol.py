...
from atlas.hypothesis_generator import HypothesisGenerator
...
from atlas.experimentation import (
    ExperimentDesigner,
    CodeGenerator,
    SandboxExecutor
)
...
# Stage 4: Publication
from atlas.publication import PaperGenerator
...
from atlas.brainstorming import BrainstormEngine
...
class ATLASProtocol:
    ...
    async def run_research(
        ...
        print(f"\n{'='*80}")
        print(f"ATLAS AUTONOMOUS RESEARCH SYSTEM")
        print(f"{'='*80}")
        print(f"\nTopic: {topic}")
    ...
    def run_qap_with_optilibria(
        ...
        *,
        ...
        """Optional helper: solve a QAP instance via Optilibria.
        ...
        wired into the main ATLAS pipeline by default.
        """

        try:
            from optimization_client import HAS_OPTILIBRIA, optimize_qap
        except ImportError as exc:  # pragma: no cover - optional dependency
            raise ImportError(
                "optimization_client module not available. Ensure the tal-ai "
                "workspace and Optilibria repo are on PYTHONPATH."
            ) from exc

        if not HAS_OPTILIBRIA:
            raise ImportError(
                "Optilibria is not available. Install Optilibria and its "
                "optilibria_client before calling this helper."
            )
    ...
    def _save_final_report(self, project: ResearchProject):
        ...
        content = f"""# ATLAS Research Project Report
...
*Generated by ATLAS - Autonomous Theorist & Laboratory Autonomous System*
