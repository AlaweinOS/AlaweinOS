...
def apply(target_path: str) -> dict:
    ...
    try:
        ...
        wf = workflows_root / 'optilibria-meta-compliance.yml'
        wf_content = (
            "name: Optilibria Meta Compliance\n"
            "on:\n"
            "  push:\n"
            "  pull_request:\n"
            "jobs:\n"
            ...
            "        run: |
python ORGANIZATIONS/AlaweinOS/optilibria/scripts/compliance_checker.py\n"
            "      - name: Upload compliance report\n"
            "        uses: actions/upload-artifact@v4\n"
            "        with:\n"
            "          name: optilibria-compliance\n"
            "          path: ORGANIZATIONS/AlaweinOS/optilibria/reports/compliance_report.json\n"
            "  style:\n"
            "    runs-on: ubuntu-latest\n"
            "    steps:\n"
            "      - uses: actions/checkout@v4\n"
            ...
            "        run: |
python -c \"import sys,json; sys.path.append('.meta/styles'); from style_enforcer import StyleEnforcer; r=StyleEnforcer().generate_style_report('ORGANIZATIONS/AlaweinOS/optilibria'); print(json.dumps(r,indent=2)); open('ORGANIZATIONS/AlaweinOS/optilibria/reports/style_report_ci.json','w').write(json.dumps(r))\"\n"
            "      - name: Fail if compliance below threshold\n"
            "        run: |
python - <<'PY'\nimport json, sys\nwith open('ORGANIZATIONS/AlaweinOS/optilibria/reports/style_report_ci.json') as f:\n    r=json.load(f)\nrate=r.get('compliance_rate',0)\nprint(f'Compliance rate: {rate}')\nsys.exit(0 if rate>=90 else 1)\nPY\n"
            "      - name: Upload style report\n"
            "        uses: actions/upload-artifact@v4\n"
            "        with:\n"
            "          name: optilibria-style\n"
            "          path: ORGANIZATIONS/AlaweinOS/optilibria/reports/style_report_ci.json\n"
        )
...
def main() -> None:
    ...
    parser.add_argument("--target", dest="target", default=str(Path("ORGANIZATIONS") / "AlaweinOS" / "optilibria"))
