"""
ATLAS adapter for Optilibria optimization services

Provides integration between ATLAS agents and Optilibria's optimization capabilities.
"""
...
from optilibria.adapters import get_adapter
from optilibria.ai.method_selector import MethodSelector
from optilibria.core.interfaces import StandardizedProblem, ValidationResult
from optilibria.integrations.atlas.config import ATLASConfig
from optilibria.integrations.atlas.task_queue import (
    OptimizationTask,
...
class ATLASOptimizationAdapter:
    """
    Adapter that enables ATLAS agents to request optimization services from Optilibria
    ...
    def __init__(self, config: Optional[ATLASConfig] = None):
        ...
        logger.info(f"Initialized ATLAS adapter with agent ID: {self.config.agent_id}")
    ...
    def submit_optimization_request(
        ...
        """
        Submit optimization request from ATLAS agent
        ...
        return {
            ...
            "redis_blackboard_key": f"atlas:blackboard:{agent_id}:optilibria:{task_id}",
    ...
    def get_agent_tasks(
        ...
        """
        Get all tasks for a specific ATLAS agent
