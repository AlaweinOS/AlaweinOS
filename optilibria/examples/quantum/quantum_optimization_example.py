...
with Optilibria's quantum integration.
...
def demonstrate_qubo_conversion():
    ...
    from optilibria.quantum.adapters import QUBOConverter
...
def demonstrate_ising_encoding():
    ...
    from optilibria.quantum.adapters import QUBOConverter, IsingEncoder
...
def demonstrate_problem_validation():
    ...
    from optilibria.quantum.validators import QuantumProblemValidator
...
def demonstrate_qubit_estimation():
    ...
    from optilibria.quantum.validators import QubitEstimator
    from optilibria.core.interfaces import StandardizedProblem
    ...
    for method_name, config in methods:
        ...
        try:
            if method_name == 'quantum_annealing':
                from optilibria.quantum.methods import quantum_annealing_optimize
                result = quantum_annealing_optimize(problem, config)
            elif method_name == 'qaoa':
                from optilibria.quantum.methods import qaoa_optimize
                result = qaoa_optimize(problem, config)
            elif method_name == 'vqe':
                from optilibria.quantum.methods import vqe_optimize
...
def demonstrate_complete_workflow():
    ...
    from optilibria.quantum import check_quantum_availability
    ...
    if not check_quantum_availability():
        ...
        print("Install with: pip install optilibria[quantum]")
        print()

    from optilibria.quantum.adapters import QuantumProblemAdapter
    from optilibria.quantum.validators import QuantumProblemValidator, QubitEstimator
    from optilibria.core.interfaces import StandardizedProblem
...
def main():
    ...
    print("\n" + "=" * 60)
    print("OPTILIBRIA QUANTUM INTEGRATION DEMONSTRATION")
    print("=" * 60)
