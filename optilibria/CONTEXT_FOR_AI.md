# Optilibria - Context for AI Assistants
...
Optilibria is a **universal optimization framework** providing a unified interface for solving optimization problems across different domains. Currently focused on combinatorial optimization (QAP, TSP) with extensible architecture for continuous, multi-objective, and constraint optimization.
...
│
└── qaplibria/              # QAP convenience layer
...
Optilibria follows an **explicit exception handling policy** documented in planning docs:
...
Optilibria uses **optional dependency groups** to keep core lightweight:
...
pip install optilibria[dev]

# With quantum support
pip install optilibria[quantum]

# With ML/hyperparameter tuning
pip install optilibria[ml]

# With theorem prover (formal verification)
pip install optilibria[theorem]

# All optional dependencies
pip install optilibria[dev,quantum,ml,docs,theorem]
```
...
Optilibria is in **pre-publication status** with papers in preparation:
...
# Run with coverage
pytest --cov=optilibria --cov-report=term
...
*This context file is the single source of truth for AI assistants working on Optilibria. Always consult this file before making significant changes.*
