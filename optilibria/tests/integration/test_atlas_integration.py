"""
Integration tests for ATLAS-Optilibria integration

Tests the integration between MEZAN/ATLAS and Optilibria optimization framework.
"""
...
from optilibria.integrations.atlas.task_queue import OptimizationTask, TaskStatus
...
@pytest.fixture
def adapter(config):
    """ATLAS adapter with mocked components"""
...
def agent(config, redis_client):
    """Optilibria agent with mocked Redis"""
    with patch('redis.Redis.from_url', return_value=redis_client):
...
class TestATLASConfig:
    ...
    def test_default_config(self):
        ...
        assert config.agent_id == "optilibria-agent"
    ...
    def test_config_from_env(self, monkeypatch):
        ...
        monkeypatch.setenv("OPTILIBRIA_AGENT_ID", "test-agent-env")
        monkeypatch.setenv("OPTILIBRIA_DEFAULT_METHOD", "genetic_algorithm")
        monkeypatch.setenv("OPTILIBRIA_ENABLE_GPU", "false")
...
class TestATLASOptimizationAdapter:
    """Test ATLAS optimization adapter"""
...
class TestOptilibriaAgent:
    """Test ATLAS-compliant agent"""
