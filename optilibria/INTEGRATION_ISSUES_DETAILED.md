# Optilibria Integration Issues - Detailed Technical Report
...
This document contains detailed technical information about critical API and integration issues found in Optilibria.
...
**File:** `optilibria_client.py` (Line 18-28)
```python
try:
    ...
    raise ImportError(
        "The 'optilibria' Python package is not installed. "
        "Install it in this environment (e.g. 'pip install -e .') "
        "before using optilibria_client."
    ) from exc
```

**File:** `optilibria/__init__.py` (Actual exports)
```python
...
1. Client tries to import `optimize` from optilibria
2. `optilibria/__init__.py` does NOT export `optimize`
3. Runtime error when importing optilibria_client

### Error on Import
```
>>> from optilibria_client import optimize_problem
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
  File "optilibria_client.py", line 19, in <module>
    from optilibria import optimize as _optimize
ImportError: cannot import name 'optimize' from 'optilibria'
```

### What Needs to Be Implemented

Create `optilibria/core/optimizer.py`:
...
Then update `optilibria/__init__.py`:
```python
...
## Critical Issue #2: Missing qaplibria Module

### Current State

**File:** `optilibria_client.py` (Line 22)
```python
from optilibria.qaplibria import optimize_qap as _optimize_qap
```

**Actual Status:** Module `optilibria/qaplibria.py` DOES NOT EXIST

### Error on Import
```
>>> from optilibria_client import optimize_qap
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
  File "optilibria_client.py", line 22, in <module>
    from optilibria.qaplibria import optimize_qap as _optimize_qap
ModuleNotFoundError: No module named 'optilibria.qaplibria'
```

### What Needs to Be Implemented

Create `optilibria/qaplibria.py`:
...
**Directory:** `optilibria/methods/baselines/`
```
baselines/
├── (EMPTY - no files)
```
...
Create 5 files in `optilibria/methods/baselines/`:
...
import numpy as np
from optilibria.core.interfaces import StandardizedProblem
...
**Location:** `optilibria/methods/novel/fft_laplace.py`
...
- `optilibria/api/` - Flask/FastAPI service
...
| Missing `qaplibria` module | P0 | Create `qaplibria.py` | 1-2 hours |
...
- [ ] `import optilibria` succeeds
- [ ] `from optilibria import optimize` succeeds
- [ ] `from optilibria import optimize_qap` succeeds
- [ ] `from optilibria_client import optimize_problem` succeeds
- [ ] All 138 tests pass
