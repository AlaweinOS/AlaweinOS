...
- Scope: compatible with ATLAS (research automation) and TURING (product automation). The Libria Suite supplies domain solvers, surfaced as “*Flow” frontends (QAPFlow, WorkFlow, AllocFlow, GraphFlow, MetaFlow, DualFlow, EvoFlow).
...
- Backend adapters: translate standardized JSON schemas to specific solver backends (e.g., `qaplibria_backend` to a modular QAP solver repo).
...
- Meta-learning: `MetaLibria` supports clustering, Elo ratings, and UCB selection for solver tournaments.
...
- Backend adapter (QAP): `Libria/libria-meta/qaplibria_backend.py`
