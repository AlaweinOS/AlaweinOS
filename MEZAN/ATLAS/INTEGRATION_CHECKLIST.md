# ATLAS ↔ Libria Integration Checklist
...
### ATLAS Core (Claude Instance 2 - This)
...
- [ ] Libria QAPLibria solver ready
- [ ] Libria FlowLibria solver ready
...
#### 2. QAPLibria Integration (Agent-Task Assignment)
...
#### 3. FlowLibria Integration (Workflow Routing)
...
- [ ] Verify ATLAS writes agent state to Redis
...
- [ ] Implement resource allocation integration (AllocLibria)
...
- [ ] GraphLibria: Communication topology
...
- [ ] Apply best architecture to ATLAS
...
def test_qap_agent_assignment():
    """Test QAPLibria agent-task assignment"""
    ...
    for task in tasks:
        ...
        assert agent_id in atlas.agents

    # Verify uses Libria (not mock)
    assert atlas.libria_router is not None
    assert not isinstance(atlas.libria_router, MockLibriaRouter)
```

### Test 2: Workflow Routing (Week 2)
```python
def test_flow_workflow_routing():
    """Test FlowLibria workflow routing"""
    atlas = ATLASEngine(libria_enabled=True)
...
- [x] QAPLibria assignment working end-to-end
- [x] FlowLibria routing working in workflows
...
**From**: Claude Instance 2 (ATLAS)
**To**: Claude Instance 1 (Libria)

**Status**: ATLAS Engine Week 1 complete. Ready for integration.
