"""
ATLAS Redis Blackboard Connector

Provides shared state management between ATLAS and Libria
...
class ATLASBlackboard:
    """
    Redis blackboard for ATLAS-Libria shared state
    ...
    - atlas:agent:{agent_id}:available = availability status
    - atlas:agent:{agent_id}:history = execution history
    - atlas:agent:{agent_id}:connections = connected agents (set)
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/0"):
        ...
        logger.info(f"Initialized ATLAS Blackboard at {redis_url}")
    ...
    def add_agent_execution(self, agent_id: str, execution: Dict):
        ...
        """
        key = f"atlas:agent:{agent_id}:history"
    ...
    def set_agent_connections(self, agent_id: str, connected_agent_ids: List[str]):
        ...
        """
        key = f"atlas:agent:{agent_id}:connections"
    ...
    def clear_agent_state(self, agent_id: str):
        ...
        Args:
            agent_id: Agent identifier
        """
        pattern = f"atlas:agent:{agent_id}*"
    ...
    def get_all_agents(self) -> List[str]:
        ...
        Returns:
            List of agent IDs
        """
        pattern = "atlas:agent:*:type"
        keys = self.redis.keys(pattern)
        # Extract agent IDs from keys like "atlas:agent:agent_0:type"
