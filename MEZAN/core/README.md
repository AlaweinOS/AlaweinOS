...
│   (ATLAS, Redis, Event Bus, API Gateway)       │
└────────────────┬────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────┐
...
factory = OptimizerFactory(config={
    "feature_flags": {
        "enable_qap_libria": True,  # Enable QAPLibria
        "enable_gpu": False,
    }
})
...
|--------------|-------------|---------------|--------|
| `QAP` | Agent-task assignment with synergies | QAPLibria | 🔧 In Development |
| `FLOW` | Confidence-aware workflow routing | FlowLibria | 📋 Planned |
| `ALLOC` | Budget-constrained resource allocation | AllocLibria | 📋 Planned |
| `GRAPH` | Agent network topology optimization | GraphLibria | 📋 Planned |
| `DUAL` | Adversarial min-max robust optimization | DualLibria | 📋 Planned |
| `EVO` | Multi-objective evolutionary optimization | EvoLibria | 📋 Planned |
| `META` | Meta-learning for algorithm selection | MetaLibria | 📋 Planned |
...
```yaml
enable_qap_libria: false      # Enable QAPLibria
enable_flow_libria: false     # Enable FlowLibria
enable_alloc_libria: false    # Enable AllocLibria
enable_graph_libria: false    # Enable GraphLibria
enable_dual_libria: false     # Enable DualLibria
enable_evo_libria: false      # Enable EvoLibria
enable_meta_libria: false     # Enable MetaLibria
```
...
- 🔧 QAPLibria handles agent assignment
- 🔧 Fallback to heuristic if QAPLibria fails

**Phase 3:** Enable additional solvers incrementally
- 📋 FlowLibria, AllocLibria, etc.
...
### ATLAS Agent Assignment

**Before (V4.0.0):**
```python
# ATLAS/atlas-core/src/atlas/intelligent_mezan.py
...
from MEZAN.ATLAS.atlas-core.src.atlas.event_bus import publish_event
...
- **MEZAN/ATLAS/START_HERE.md** - ATLAS orchestration
- **optilibria/README.md** - General optimization framework
