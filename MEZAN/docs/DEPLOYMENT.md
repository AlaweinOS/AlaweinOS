...
MEZAN ATLAS is a production-grade multi-agent orchestration platform designed for enterprise-scale deployments. This guide covers deployment strategies, configurations, and operational procedures.
...
|---------|---------|------|----------|
| atlas-api | Main API server | 8080 | 2-10 |
...
# View logs
docker-compose logs -f atlas-api-dev
```
...
sudo chown $USER:$USER /opt/atlas
cd /opt/atlas
...
kubectl port-forward -n atlas svc/atlas-api 8080:8080
```
...
docker tag mezan-atlas:latest $ECR_REGISTRY/mezan-atlas:latest
docker push $ECR_REGISTRY/mezan-atlas:latest

# Deploy with CloudFormation
aws cloudformation create-stack \
    --stack-name atlas-production \
    --template-body file://cloudformation/atlas-ecs.yml \
...
gcloud builds submit --tag gcr.io/$PROJECT_ID/mezan-atlas

# Deploy to Cloud Run
gcloud run deploy atlas-api \
    --image gcr.io/$PROJECT_ID/mezan-atlas \
    --platform managed \
    --region us-central1 \
...
| `POSTGRES_USER` | PostgreSQL username | No | atlas |
...
docker-compose exec postgres psql -U atlas -f /docker-entrypoint-initdb.d/init.sql
```
...
docker-compose exec atlas-api ping -c 3 redis
docker-compose exec atlas-api ping -c 3 postgres
...
# Check logs
docker-compose logs --tail=100 atlas-api
```
