...
**Managing Parallel Development: ATLAS (Claude 2) + Libria (Claude 1)**
...
|-----------|----------------|-----------|----------|----------|
| **Libria Solvers** | Claude 1 (This) | `/Libria/` | 🔴 CRITICAL | MetaLibria: Mar 31 |
| **ATLAS Engine** | Claude 2 (Other) | `/ATLAS/` | 🟡 HIGH | Week 6 |
...
1. CRITICAL: MetaLibria implementation (March 31 deadline)
   - Start with code from PROMPT_MetaLibria.md
...
2. HIGH: Setup libria-integration/ package
   ...
   - Create feature extractors for ATLAS ResearchAgent → features
...
**To Claude Instance 2 (ATLAS - New Instance)**:
```
I need you to implement the ATLAS Engine that will integrate with the Libria Suite.

Background:
- ATLAS = 40+ research agents for multi-agent AI orchestration
...
1. Week 1: Implement ATLAS Core
   - Create atlas-core/atlas_core/engine.py with ATLASEngine class
   - Create atlas-core/atlas_core/agent.py with ResearchAgent base class
...
3. Week 2: Integrate with Libria
   ...
   - Use for agent-task assignment (QAPLibria)
   - Use for workflow routing (FlowLibria)
...
See IMPLEMENTATION_MASTER_PLAN.md Section "2.1 ATLAS Core Architecture" for complete code templates.
```
...
# 3. Review logs
docker-compose logs --tail=50 atlas-core
...
- [ ] libria-meta/ MetaLibria implementation started
...
**Integration** (Both):
- [ ] LibriaRouter can be imported by ATLAS
...
```python
# Run this from ATLAS Engine (Claude 2)
...
print("✅ Dialectical workflow with FlowLibria routing working")
```
...
**Impact**: ATLAS needs to add model_type to ResearchAgent dataclass
...
**Approval Needed**: Claude 2 (ATLAS)
```
...
**Problem**: "LibriaRouter import fails in ATLAS"

**Solution**:
```python
# In ATLAS code, add path
import sys
...
# From ATLAS:
redis-cli SET atlas:test "hello from ATLAS"

# From Libria:
redis-cli GET atlas:test
# Should return "hello from ATLAS"
```
...
# In Libria:
self._extract_agent_features(agents)  # Should match ATLAS format
...
- ✅ MetaLibria 60% implemented
- ✅ LibriaRouter skeleton

**In Progress**:
- ⏳ MetaLibria baselines (40% done)
- ⏳ QAPLibria implementation (20% done)

**Blocked**:
- None

**Next Week**:
- Complete MetaLibria baseline implementations
- Start QAPLibria cost prediction
- Begin FlowLibria LinUCB
...
- Integrate QAPLibria for assignment
...
git init
git add .
git commit -m "Initial commit: Itqān Suite with ATLAS + Libria"

# Daily workflow for YOU (not Claude instances)
# After Claude 1 makes changes:
git add Libria/
git commit -m "Claude 1: Implemented MetaLibria tournament selection"
...
3. Test in isolation (without ATLAS dependency)
4. Use mock data from superprompt examples

### If Claude 2 gets stuck:
1. Check IMPLEMENTATION_MASTER_PLAN.md Section 2.1 for ATLAS architecture
...
# View logs
docker-compose logs -f [atlas-core|libria-api|redis|postgres]

# Run tests
docker-compose exec atlas-core pytest
...
- ✅ Dialectical workflow routing via FlowLibria
- ✅ < 5 integration conflicts total

**Week 4**:
- ✅ All 6 integration points working
- ✅ 40+ ATLAS agents operational
- ✅ MetaLibria benchmarking started

**Week 12**:
- ✅ MetaLibria paper submitted (March 31)
- ✅ Full ATLAS+Libria suite deployed
...
Both Claude instances are fully equipped to build their components. Just coordinate at the integration boundaries and you'll have a complete ATLAS + Libria suite in 12 weeks.
