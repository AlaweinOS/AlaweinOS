...
Scope: grounded in current repo (ATLAS, Libria, docs, CI). Items are actionable and non-destructive unless noted.
...
15. Add tests for `MetaLibria.fit/select/update` happy paths with mock solvers.
...
47. Introduce config dataclasses for baselines and MetaLibria.
...
54. Make FeatureExtractor pluggable via protocol in MetaLibria.
...
61. Add ATLAS↔Libria integration notes in `docs/`.
...
92. Ensure ATLAS unit tests run in CI hygiene job.
93. Add README if missing under `ATLAS/` with quick start.
94. Document engine/agent architecture in ATLAS docs.
95. Add example showing Libria selection driving ATLAS solver choice.
96. Standardize logging configuration between ATLAS and Libria.
97. Add type hints to key ATLAS modules (agent, engine).
98. Add `ruff`/`black` to ATLAS CI as well.
99. Add lightweight perf test for ATLAS pipelines.
100. Add ATLAS docs link from root README.
