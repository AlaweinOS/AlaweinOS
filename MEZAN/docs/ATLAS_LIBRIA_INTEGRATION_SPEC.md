# ATLAS ↔ Libria Integration Specification

**Version**: 1.0
**Date**: 2026-01-17
**Purpose**: Define integration contract between ATLAS Engine (Claude Instance 2) and Libria Suite (Claude Instance 1)
...
### 2.1 Agent-Task Assignment (QAPLibria)

**ATLAS Use Case**: Assign research tasks to 40+ agents optimally

**API Call**:
```python
# In ATLAS Engine (Claude Instance 2)
...
**Data Flow**:
1. ATLAS provides list of agents + tasks
2. LibriaRouter extracts agent features (skill, workload, history)
3. QAPLibria computes optimal assignment
4. ATLAS receives agent-task mapping
5. ATLAS assigns tasks accordingly
...
def solve_assignment(
    ...
    """
    Args:
        agents: List of ATLAS research agents
...
### 2.2 Workflow Routing (FlowLibria)

**ATLAS Use Case**: Route dialectical workflow steps to best agents with confidence

**API Call**:
```python
# In ATLAS dialectical workflow
...
**Data Flow**:
1. ATLAS provides workflow state + available agents
2. FlowLibria extracts workflow features
3. LinUCB selects agent maximizing quality + calibration
4. Returns agent_id + confidence estimate
5. ATLAS routes to selected agent
...
### 2.3 Resource Allocation (AllocLibria)

**ATLAS Use Case**: Allocate compute/memory/bandwidth to agents under budget
...
**Data Flow**:
1. ATLAS provides agents + total budget
2. AllocLibria builds resource request matrix
3. Constrained Thompson Sampling solves allocation
4. Returns resource allocation per agent
5. ATLAS applies resource limits
...
### 2.4 Topology Optimization (GraphLibria)

**ATLAS Use Case**: Optimize agent communication network for information flow
...
**Data Flow**:
1. ATLAS provides agent state history (T × n × d)
2. GraphLibria estimates pairwise mutual information
3. Spectral optimization finds best topology
4. Returns adjacency matrix
5. ATLAS updates communication graph
...
### 2.5 Workflow Validation (DualLibria)

**ATLAS Use Case**: Pre-deployment adversarial testing of dialectical workflows
...
**Data Flow**:
1. ATLAS provides workflow + safety spec + test cases
2. DualLibria runs red-team attacks (PGD, genetic, beam)
3. Blue team robustifies workflow
4. Returns validation report + robustified workflow
5. ATLAS deploys robustified version
...
### 2.6 Architecture Evolution (EvoLibria)

**ATLAS Use Case**: Discover optimal agent team architectures via MAP-Elites
...
**Data Flow**:
1. ATLAS provides task distribution
2. EvoLibria evolves agent architectures via MAP-Elites
3. Returns diverse archive of architectures
4. ATLAS selects architecture for current needs
...
def evolve_architecture(
    ...
    """
    Args:
        task_distribution: Sample of ATLAS tasks for evaluation
...
@dataclass
class ResearchAgent:
    """ATLAS research agent representation"""
    agent_id: str
...
@dataclass
class ResearchTask:
    """ATLAS research task representation"""
    task_id: str
...
### 3.3 Agent State Schema (for GraphLibria)
...
Both ATLAS and Libria share state via Redis:

### 4.1 Agent State Keys

```
# Agent metadata
atlas:agent:{agent_id}:type = "synthesis"
atlas:agent:{agent_id}:skill_level = 0.85
atlas:agent:{agent_id}:workload = 3
atlas:agent:{agent_id}:available = True

# Agent history (for learning)
atlas:agent:{agent_id}:history = List[execution_record]

# Agent connections (from GraphLibria topology)
atlas:agent:{agent_id}:connections = Set[agent_ids]
```

### 4.2 Libria Solver State Keys

```
# QAPLibria cost predictions
libria:qap:cost_matrix = serialized_matrix

# MetaLibria Elo ratings
libria:meta:elo:{solver_id} = 1542.3

# FlowLibria LinUCB parameters
libria:flow:A:{agent_id} = serialized_matrix
libria:flow:b:{agent_id} = serialized_vector

# GraphLibria topology
...
### 5.1 For Claude Instance 2 (ATLAS)

**Week 1**:
- [ ] Implement `atlas-core/atlas_core/engine.py` with ATLASEngine class
- [ ] Implement `atlas-core/atlas_core/agent.py` with ResearchAgent base class
...
**Week 3**:
- [ ] Implement workflow routing via FlowLibria
- [ ] Add resource allocation via AllocLibria
- [ ] Test end-to-end: ATLAS task → Libria solver → ATLAS execution

**Week 4**:
- [ ] Add topology optimization via GraphLibria
- [ ] Implement periodic topology updates
- [ ] Add workflow validation via DualLibria

**Week 5-6**:
- [ ] Expand to 40+ research agents
- [ ] Implement architecture evolution via EvoLibria
...
- [ ] Implement MetaLibria (PRIORITY for March 31)

**Week 2**:
- [ ] Implement QAPLibria with agent-task assignment features
- [ ] Test integration with ATLAS mock data
- [ ] Implement FlowLibria with workflow routing

**Week 3-4**:
- [ ] Implement AllocLibria with resource allocation
- [ ] Implement GraphLibria with topology optimization
- [ ] Test all integrations with ATLAS Engine

**Week 5-12**:
- [ ] Complete remaining solvers (DualLibria, EvoLibria)
- [ ] Benchmarking on ATLAS workloads
...
- Test feature extraction from ATLAS schemas

### 7.2 Integration Tests (Coordinated)

**Scenarios**:
1. **Agent Assignment**: ATLAS creates 10 agents + 5 tasks → QAPLibria assigns → ATLAS executes
2. **Workflow Routing**: ATLAS runs thesis-antithesis-synthesis → FlowLibria routes each step
3. **Resource Allocation**: ATLAS requests resources → AllocLibria allocates → ATLAS applies limits
4. **Topology Optimization**: ATLAS collects state history → GraphLibria optimizes → ATLAS updates graph
...
assignment = router.solve_assignment(
    agents=list(atlas.agents.values()),
    tasks=[task]
)

assigned_agent = atlas.agents[assignment['agent_id']]

# Execute dialectical workflow using FlowLibria routing
workflow_result = atlas.execute_workflow(
    workflow_type="thesis_antithesis_synthesis",
    inputs={'topic': 'neural_architecture_search'}
)

# Optimize topology weekly using GraphLibria
if week_number % 1 == 0:
    agent_states = atlas.get_agent_state_history(window=100)
    optimal_topology = router.optimize_topology(agent_states)
    atlas.apply_communication_topology(optimal_topology)
...
class LibriaRouter:
    def __init__(self):
        ...
        self.qap_solver = QAPLibria()
        self.flow_solver = FlowLibria(n_agents=40)
        self.graph_solver = GraphLibria(n_agents=40)

        # Train on historical ATLAS data
...
**Next Steps**:
1. Claude Instance 2: Begin ATLAS Engine implementation (Week 1)
2. Claude Instance 1: Begin Libria core + MetaLibria (Week 1)
