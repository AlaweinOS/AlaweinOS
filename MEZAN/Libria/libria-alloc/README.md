# AllocLibria - Budget-Constrained Resource Allocation

**Thompson Sampling for optimal resource allocation under uncertainty in ATLAS multi-agent systems**

## Overview

AllocLibria solves the budget-constrained resource allocation problem using Thompson Sampling, a Bayesian bandit algorithm that balances exploration and exploitation.
...
AllocLibria uses Thompson Sampling with Beta-Bernoulli conjugate priors:
...
```python
class AllocLibriaSolver(OptimizerInterface):
    def __init__(
        self,
...
# Factory selects AllocLibria for ALLOC problems
