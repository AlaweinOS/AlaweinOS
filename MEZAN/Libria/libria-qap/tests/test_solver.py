"""
Comprehensive tests for QAPLibria solver
...
class TestQAPLibriaSolver:
    """Test suite for QAPLibria solver"""
    ...
    def solver_sa(self):
        ...
        return QAPLibriaSolver(config={"algorithm": "simulated_annealing", "max_iterations": 100})

    @pytest.fixture
    def solver_ga(self):
        """Solver with Genetic Algorithm"""
        return QAPLibriaSolver(config={"algorithm": "genetic", "max_iterations": 100})
    ...
    def test_deterministic_with_seed(self, small_qap_problem):
        ...
        np.random.seed(42)
        solver1 = QAPLibriaSolver(config={"algorithm": "simulated_annealing", "max_iterations": 50})
        solver1.initialize()
        result1 = solver1.solve(small_qap_problem)

        np.random.seed(42)
        solver2 = QAPLibriaSolver(config={"algorithm": "simulated_annealing", "max_iterations": 50})
        solver2.initialize()
...
class TestQAPLibriaIntegration:
...
class TestQAPLibriaBenchmarks:
    """Performance benchmarks for QAPLibria"""
    ...
    def test_scaling_with_problem_size(self, n):
        ...
        solver = QAPLibriaSolver(config={"max_iterations": 100})
        solver.initialize()
