# QAPLibria - Quadratic Assignment Problem Solver

**GPU-accelerated solver for optimal agent-task assignment in ATLAS multi-agent orchestration**

## Overview

QAPLibria solves the Quadratic Assignment Problem (QAP), optimizing the assignment of N agents to N tasks while minimizing the total cost considering pairwise interactions (synergies).
...
QAPLibria implements two algorithms:
...
```python
solver = QAPLibriaSolver(enable_gpu=True)
```
...
```python
from MEZAN.ATLAS.atlas_core.src.atlas.optimization_integration import optimize_atlas_agent_assignment
...
QAPLibria includes integration with 20+ QAPLIB benchmark instances:
...
```python
class QAPLibriaSolver(OptimizerInterface):
    def __init__(
        self,
...
QAPLibria emits Prometheus metrics:
