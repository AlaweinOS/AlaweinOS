...
**Optimal workflow routing for ATLAS research pipelines with uncertainty quantification**

## Overview

FlowLibria optimizes routing through research workflows by finding paths that maximize cumulative confidence while respecting workflow constraints.
...
FlowLibria uses Dijkstra's shortest path algorithm with modifications:
...
```python
solver = FlowLibriaSolver(config={
...
```python
class FlowLibriaSolver(OptimizerInterface):
    def __init__(
        self,
...
# Factory automatically selects FlowLibria for FLOW problems
