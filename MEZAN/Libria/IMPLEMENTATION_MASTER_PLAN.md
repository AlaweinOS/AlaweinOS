...
This document provides the master plan for implementing all 7 Libria solvers in parallel while respecting the critical path (MetaLibria - March 31, 2025 deadline). It coordinates multi-stream development, ATLAS integration, and inter-Claude collaboration.
...
- **Claude Instance 2** (ATLAS): Parallel instance - ATLAS Engine development
...
│
├── ATLAS/                          # ← ATLAS Engine (Claude Instance 2)
│   ├── atlas-core/
│   │   ├── atlas_core/
│   │   │   ├── __init__.py
...
│   │   └── setup.py
│   │
│   ├── atlas-agents/               # Research agents (40+)
│   │   ├── synthesis_agent.py
│   │   ├── literature_review_agent.py
│   │   ├── hypothesis_generation_agent.py
│   │   └── ...
│   │
│   ├── atlas-workflows/            # Dialectical workflows
│   │   ├── thesis_antithesis_synthesis.py
│   │   ├── multi_perspective_analysis.py
│   │   └── quality_gates.py
│   │
│   └── atlas-api/                  # API for Libria integration
│       ├── libria_interface.py     # Libria → ATLAS adapter
...
│   │   └── setup.py
│   │
│   ├── libria-meta/                # MetaLibria (PRIORITY)
│   │   ├── libria_meta/
│   │   │   ├── __init__.py
│   │   │   ├── meta_solver.py
│   │   │   ├── elo_tracker.py
...
│   ├── libria-evo/
│   │
│   ├── libria-integration/         # ATLAS ↔ Libria integration
│   │   ├── libria_integration/
│   │   │   ├── __init__.py
│   │   │   ├── atlas_adapter.py    # Libria → ATLAS
│   │   │   ├── solver_router.py    # Route tasks to solvers
│   │   │   └── state_sync.py       # Sync with ATLAS blackboard
│   │   └── tests/
│   │
...
│       └── grafana-dashboards/
│
└── shared/                         # Shared between ATLAS and Libria
...
## 2. ATLAS Structure Recommendations

### 2.1 ATLAS Core Architecture (For Claude Instance 2)

Since you'll be prompting another Claude to work on ATLAS, here's my recommended structure:

```python
# ATLAS/atlas-core/atlas_core/engine.py
...
@dataclass
class AgentConfig:
    """Configuration for ATLAS research agent"""
    agent_id: str
...
class ResearchAgent:
    """Base class for ATLAS research agents"""
...
class ATLASEngine:
    ...
    def register_agent(self, agent: ResearchAgent):
        """Register research agent with ATLAS"""
        self.agents[agent.config.agent_id] = agent

        # Store agent state in Redis blackboard
        self.redis.hset(
            f"atlas:agent:{agent.config.agent_id}",
            mapping={
    ...
    def assign_task(self, task: Dict) -> Optional[str]:
        ...
        - MetaLibria: Agent selection meta-learning
        - FlowLibria: Workflow routing
        """
        if self.libria_enabled:
            # Use Libria QAPLibria for optimal assignment
    ...
    def execute_workflow(
        ...
        - adversarial_validation (uses DualLibria)
        """
        if workflow_type == "thesis_antithesis_synthesis":
            ...
            # Use DualLibria for workflow validation
            if self.libria_enabled:
    ...
    def _dialectical_workflow(self, inputs: Dict) -> Dict:
        ...
        # Step 2: Antithesis (use FlowLibria for routing)
        if self.libria_enabled:
    ...
    def get_agent_state_history(self, window: int = 100):
        """
        Get agent state history for GraphLibria

        Returns: (T × n × d) array of agent states over time
        """
        states = []
        for agent_id in self.agents:
            agent_history = self.redis.lrange(f"atlas:agent:{agent_id}:history", 0, window-1)
    ...
    def optimize_communication_topology(self):
        """
        Use GraphLibria to optimize agent communication network
        ...
        # Use GraphLibria to optimize topology
    ...
    def _apply_topology(self, adjacency_matrix):
        ...
        for i, agent_id_i in enumerate(self.agents.keys()):
            ...
            self.redis.sadd(
                f"atlas:agent:{agent_id_i}:connections",
                *connected_agents
            )
```

### 2.2 Libria ↔ ATLAS Integration Layer
...
import sys
sys.path.append('../../ATLAS/atlas-core')

from atlas_core.engine import ATLASEngine, ResearchAgent

class LibriaRouter:
    """
    Routes ATLAS tasks to appropriate Libria solvers

    Solver Routing:
    - QAPLibria: Agent-task assignment
    - MetaLibria: Solver selection for tasks
    - FlowLibria: Workflow routing with confidence
    - AllocLibria: Resource allocation to agents
    - GraphLibria: Communication topology optimization
    - DualLibria: Workflow adversarial validation
    ...
    def __init__(self):
        ...
        self.meta_solver = MetaLibria(solvers=[...])  # Add Libria solvers
        self.flow_solver = FlowLibria(n_agents=40)  # ATLAS has 40+ agents
        self.alloc_solver = AllocLibria(n_agents=40, n_resource_types=3)
        self.graph_solver = GraphLibria(n_agents=40)
        self.dual_solver = DualLibria(workflow=None, safety_spec=None)
        self.evo_solver = EvoLibria()

        # Load historical data from ATLAS
    ...
    def solve_assignment(
        ...
        """Use QAPLibria for agent-task assignment"""
    ...
    def allocate_resources(
        ...
        """Use AllocLibria for resource allocation"""
    ...
    def optimize_topology(
        ...
        """Use GraphLibria for topology optimization"""
    ...
    def validate_workflow(
        ...
        """Use DualLibria for adversarial workflow validation"""
        ...
        # Update DualLibria with workflow
    ...
    def evolve_architecture(
        ...
        """Use EvoLibria for agent architecture evolution"""

        def performance_metric(architecture, task):
            # Simulate ATLAS execution with architecture
    ...
    def _extract_agent_features(self, agents: List[ResearchAgent]):
        """Extract features from ATLAS agents"""
        import numpy as np
        features = []
        for agent in agents:
            feat = [
    ...
    def _load_historical_data(self):
        """Load ATLAS execution history to train Libria solvers"""
        # Query Redis/PostgreSQL for historical executions
        # Train QAPLibria cost predictor
        # Train FlowLibria routing policy
        # etc.
        pass
```
...
openapi: 3.0.0
info:
  title: ATLAS-Libria Integration API
  version: 1.0.0
  description: API contract between ATLAS Engine and Libria Solvers
...
### Stream 1: MetaLibria (CRITICAL PATH)
...
**Deliverables**:
- Complete MetaLibria implementation
...
- QAPLibria: Weeks 1-6
- FlowLibria: Weeks 3-8

### Stream 4: ATLAS Integration Layer
...
- Week 3: Implement ATLAS adapter
- Week 4: Integration testing
- Week 5-6: End-to-end validation

### Stream 5: AllocLibria + GraphLibria

**Owner**: This Claude instance
**Priority**: 🟠 MEDIUM
**Timeline**: Weeks 3-10

### Stream 6: DualLibria + EvoLibria
