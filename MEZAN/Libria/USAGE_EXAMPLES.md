...
## Table of Contents
1. [QAPLibria - Agent-Task Assignment](#1-qaplib ria---agent-task-assignment)
2. [FlowLibria - Workflow Routing](#2-flowlibria---workflow-routing)
3. [AllocLibria - Resource Allocation](#3-alloclibria---resource-allocation)
4. [GraphLibria - Network Topology](#4-graphlibria---network-topology)
5. [DualLibria - Adversarial Robust](#5-duallibria---adversarial-robust)
6. [EvoLibria - Multi-Objective](#6-evolibria---multi-objective)
7. [MetaLibria - Algorithm Selection](#7-metalibria---algorithm-selection)
8. [Integration with ATLAS](#8-integration-with-atlas)
...
**Problem:** Assign 40 ATLAS research agents to tasks considering synergies.
...
# Solve
solver = FlowLibriaSolver(config={"confidence_threshold": 0.7})
solver.initialize()
result = solver.solve(problem)
...
## 3. AllocLibria - Resource Allocation
...
solver = AllocLibriaSolver(config={
...
## 4. GraphLibria - Network Topology
...
# Solve
solver = GraphLibriaSolver(config={"objective": "minimize_entropy"})
solver.initialize()
result = solver.solve(problem)
...
## 5. DualLibria - Adversarial Robust
...
# Solve
solver = DualLibriaSolver(config={
...
## 7. MetaLibria - Algorithm Selection
...
# MetaLibria automatically selects QAPLibria
meta_solver = MetaLibriaSolver()
meta_solver.initialize()
result = meta_solver.solve(qap_problem)

print(f"Selected solver: {result.solution['selected_solver']}")  # "QAPLibria"
...
print(f"Selected solver: {result.solution['selected_solver']}")  # "FlowLibria"

# MetaLibria learns from experience
...
# optimizer is now QAPLibriaSolver (if enabled) or HeuristicFallback (if disabled)
...
```python
from atlas.optimization_integration import optimize_atlas_agent_assignment
...
if result["status"] == "success":
    ...
    # Use assignment in ATLAS workflow
