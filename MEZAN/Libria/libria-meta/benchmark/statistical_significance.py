...
Performs statistical tests to validate MetaLibria's performance:
...
# Compare MetaLibria (optimal) vs each baseline
ml_optimal_regrets = regret_matrix.loc['MetaLibria (optimal)'].values

comparisons = []
for method in methods:
    if method == 'MetaLibria (optimal)':
        continue
...
print("\nMetaLibria (optimal) vs Baselines:")
...
for scenario in scenarios:
    ...
    if len(ml_optimal) > 0:
        ...
        ml_rank = list(scenario_df['Method']).index('MetaLibria (optimal)') + 1

        # Find best method
        best = scenario_df.iloc[0]

        if best['Method'] == 'MetaLibria (optimal)':
            print(f"  ✅ MetaLibria WINS (regret: {ml_regret:.4f})")
        else:
...
print(f"\nMetaLibria (optimal) speed: {ml_speed:.2f}ms")
