...
1. MetaLibria's ranking across scenarios
2. Scenarios where MetaLibria excels
...
print("\n" + "="*70)
print("1. MetaLibria Ranking by Scenario")
print("="*70)
...
for scenario in scenarios:
    ...
    # Find MetaLibria optimal
    ml_optimal = scenario_df[scenario_df['Method'] == 'MetaLibria (optimal)']
    ...
    if len(ml_optimal) > 0:
        ...
        print(f"\n{scenario}:")
        print(f"  MetaLibria (optimal): Rank {rank}/{len(scenario_df)}")
        ...
        if rank == 1:
            print(f"  ✅ MetaLibria WINS!")
        elif rank <= 2:
            print(f"  🎯 MetaLibria COMPETITIVE (top 2)")
        elif rank <= 3:
            print(f"  ⚠️ MetaLibria DECENT (top 3)")
        else:
            print(f"  ❌ MetaLibria POOR (rank {rank})")
...
ml_df = df[df['Method'].str.contains('MetaLibria')].copy()
...
# 3. Scenarios where MetaLibria excels
print("\n" + "="*70)
print("3. Scenarios Where MetaLibria Excels")
print("="*70)
...
# Find MetaLibria speedup
ml_speed = speed_df['MetaLibria (optimal)']
...
ml_only = df[df['Method'].str.contains('MetaLibria')].copy()
...
for scenario in scenarios:
    ...
    default = scenario_df[scenario_df['Method'] == 'MetaLibria (default)']
    optimal = scenario_df[scenario_df['Method'] == 'MetaLibria (optimal)']
