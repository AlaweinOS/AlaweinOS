"""
Integration test for MetaLibria with ASlib benchmarks

This test demonstrates the complete workflow:
1. Create mock solvers
2. Train MetaLibria on historical data
...
from benchmark.evaluate_metalibria import ASLibEvaluator, create_mock_solver_for_aslib
...
def test_metalibria_end_to_end():
    """Test complete MetaLibria workflow"""
    print("\n" + "="*60)
    print("MetaLibria End-to-End Integration Test")
    print("="*60)
    ...
    # 2. Initialize MetaLibria
    print("\n2. Initializing MetaLibria...")
    meta = MetaLibria(
        solvers=solvers,
        n_clusters=3,
        elo_k=32.0,
...
def test_metalibria_with_aslib_framework():
    """Test MetaLibria integration with ASlib framework"""
    print("\n" + "="*60)
    print("MetaLibria + ASlib Framework Integration Test")
    print("="*60)

    # Create solvers and MetaLibria
    solvers = [
        create_mock_solver_for_aslib(f'Solver{i}', base_performance=0.5 + i*0.1)
        for i in range(3)
    ]

    meta = MetaLibria(solvers=solvers, n_clusters=2, n_tournament_rounds=2)
