# Figure Specifications for MetaLibria Paper
...
## Figure 1: MetaLibria Architecture (Methods Section)
...
**Caption**:
"MetaLibria architecture. Training phase: Cluster problem instances, run Swiss-system tournaments to update Elo ratings (global + cluster-specific). Selection phase: Extract features, assign to cluster, select algorithm via UCB using cluster Elo ratings. Selection completes in 0.15ms."
...
**Bars**: 7 methods (different colors)
  - MetaLibria (optimal) - **BLUE** (highlighted)
  - MetaLibria (default) - light blue
  - SATzilla - orange
  - AutoFolio - green
  - SMAC - red
  - Hyperband - gray
...
**Annotations**:
- Star above GRAPHS-2015 for MetaLibria: "WINS (rank 1/7)"
- Medal icons: 🥇 for rank 1, 🥈 for rank 2

**Data Source**: `results/phase2/phase2_results_summary.csv`

**Caption**:
"Per-scenario performance comparison. MetaLibria (optimal) wins on GRAPHS-2015 (graph problems) with 0.019 regret, 5.5% better than second-best (AutoFolio). Competitive on CSP-2010 (rank 2/7, 0.003 regret). Weak on SAT11-HAND and ASP-POTASSCO (rank 5/7)."
...
```
  Regret
   ...
   0.06 ┤              ● SMAC
        │         ⬤ SATzilla
   0.04 ┤    ⬤ MetaLibria (optimal)  ← PARETO OPTIMAL
        │    ⬤ MetaLibria (default)
   0.02 ┤
        │
   0.00 ┼─────┬─────┬─────┬─────┬─────┬─────┬─────
...
- MetaLibria (optimal): 0.15ms, 0.0545 regret
- SATzilla: 254ms, 0.0603 regret

**Annotations**:
- Arrow pointing to MetaLibria: "1664x faster, only 10% worse regret"
- Region shading: "Real-time region (<1ms)" for left side

**Data Source**: `results/phase2/phase2_results_summary.csv` (grouped by method)

**Caption**:
"Speed-accuracy trade-off. MetaLibria (optimal) achieves best Pareto efficiency: 1664x faster than SATzilla with only 10% higher regret. Hyperband and BOHB are fast but inaccurate (2x worse regret). MetaLibria enables real-time selection (<1ms) with competitive accuracy."

**Key Insight**: Demonstrates MetaLibria's optimal position in speed-accuracy space
