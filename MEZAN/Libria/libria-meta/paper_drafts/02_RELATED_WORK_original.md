...
MetaLibria builds on three research threads: algorithm selection, meta-learning for hyperparameter optimization, and tournament-based learning systems. We position our contributions relative to each.
...
**Limitation.** State-of-the-art algorithm selection methods achieve strong accuracy but high latency (24-254ms). MetaLibria addresses this gap, targeting sub-millisecond selection through tournament-based meta-learning.
...
**Gap.** Existing meta-learning approaches either sacrifice speed (SMAC, Auto-sklearn) or accuracy (Hyperband, BOHB). MetaLibria achieves both through tournament-based learning, which efficiently captures algorithm strengths while enabling fast selection via pre-computed Elo ratings.
...
**Our contribution.** MetaLibria introduces tournament-based meta-learning to algorithm selection, using Swiss-system tournaments for efficient algorithm ranking and dual Elo ratings (global + cluster-specific) for specialized performance tracking. This combination is novel.
...
MetaLibria achieves best regret with near-instant selection, enabled by pre-computed Elo ratings and linear-time clustering.
