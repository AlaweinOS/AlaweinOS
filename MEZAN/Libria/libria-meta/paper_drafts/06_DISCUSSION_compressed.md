...
MetaLibria achieves rank 1/7 on GRAPHS-2015 (1.9% regret), outperforming all baselines by 6-7×. Why does tournament-based selection excel on graph problems?
...
**Real-time constraint systems**: Interactive tools (product configurators, planning assistants) require <100ms total response. MetaLibria's 0.15ms overhead (vs. SATzilla's 254ms) allows hundreds of solver calls for richer interaction.
...
**Binary selection tasks**: On scenarios with few algorithms (CSP-2010: 6 algorithms), MetaLibria achieves 96.5% accuracy with 1600× speedup.
