...
We introduce MetaLibria, a tournament-based meta-learning framework for fast algorithm selection. This section describes the problem formulation, our approach's key components, and the training and selection mechanisms.
...
- **MetaLibria (default)**: k=5, λ=1.0, R=5, K=32
- **MetaLibria (optimal)**: k=3, λ=1.0, R=5, K=32
...
**Training time**: Total training (clustering + tournaments) completes in 0.1-0.5 seconds per scenario, making MetaLibria practical for retraining.
