...
def _as_qap_matrices(problem: Dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    ...
    if "A" in problem and "B" in problem:
        ...
        if A.shape[0] != A.shape[1] or B.shape[0] != B.shape[1] or A.shape != B.shape:
            raise ValueError("qaplibria_backend: 'A' and 'B' must be square and same shape")
        return A, B

    fit = problem.get("fit")
    if fit is None:
        raise ValueError("qaplibria_backend: expected 'A'/'B' or 'fit'")
    A = np.array(fit, dtype=float)
    n = A.shape[0]
    if A.shape[0] != A.shape[1]:
        raise ValueError("qaplibria_backend: 'fit' must be square (n x n)")
    if "interaction" in problem:
        B = np.array(problem["interaction"], dtype=float)
        if B.shape != A.shape:
            raise ValueError("qaplibria_backend: 'interaction' must match 'fit' shape")
    else:
...
def _try_modular_repo_solver(A: np.ndarray, B: np.ndarray, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Default guess
    default_path = "/mnt/c/Users/mesha/Documents/Projects/Active/QAPlibria/projects/QAP_modular_repo-main/src"
