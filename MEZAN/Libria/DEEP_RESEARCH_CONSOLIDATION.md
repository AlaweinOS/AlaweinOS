...
You are receiving a comprehensive information package for designing an autonomous AI research system (ATLAS/TURING) powered by 7 custom optimization solvers (Optilibria Suite).
...
- ATLAS/TURING definition, purpose, components
...
- Context in ATLAS: [how does it fit?]  
...
QAPLibria (Agent-Task Assignment)
...
EvoLibria (Architecture Auto-Discovery)
...
---------|------|----------|----------|---------|----------|------
Assignment Opt | OR-Tools | Production | Python/C++ | Apache | QAPLibria baseline | [url]
Agent Orchestration | AutoGen | Active | Python | MIT | ATLAS coordination | [url]
Include:
...
Two-account strategy (Account 1: ATLAS, Account 2: Libria)
...
- What is ATLAS/TURING? [Clear 3-sentence description]
- What is Optilibria Suite? [7 solvers, purpose, integration]
...
### 1.4 Optilibria Integration
[How solvers plug into ATLAS]
[Standard interfaces]
[Decision flow: when is each solver invoked?]

## 2. Solver Specifications (20 pages - ~3 per solver)

### 2.1 QAPLibria: Agent-Task Assignment
...
Priority: 1 (Critical - needed immediately for ATLAS agent assignment)
Dependencies: None (can start immediately)
Blocks: MetaLibria (needs this solver operational to meta-learn over it)
[REPEAT THIS STRUCTURE FOR ALL 7 SOLVERS]

2.2 FlowLibria: Workflow Routing
[Full specification as above]

2.3 AllocLibria: Resource Allocation
[Full specification as above]

2.4 GraphLibria: Network Topology
[Full specification as above]

2.5 MetaLibria: Solver Selection
[Full specification as above]

2.6 DualLibria: Adversarial Validation
[Full specification as above]

2.7 EvoLibria: Architecture Auto-Discovery
[Full specification as above]
...
3.2 ATLAS Adapter Pattern
[How ATLAS calls any solver]
[Error handling]
[Fallback strategies]
...
[Gantt chart in text]
[Paper 1 (QAPLibria): Months 1-6]
[Paper 2 (FlowLibria): Months 3-9]
[etc.]
...
Account 1 (ATLAS/TURING): Production system development
...
## Overview  
- **Duration**: 12 weeks  
- **Accounts**: 2 (ATLAS production + Libria research)  
...
## Week 1-2: Foundation + QAPLibria Deep Dive  
...
- [ ] Create ATLAS synthetic benchmark (100 agents, 50 tasks)  
- [ ] Run experiments: baseline vs. novel on QAPLIB + ATLAS scenarios  
- [ ] Analyze results: optimality gap, runtime, improvement %  
- [ ] Draft paper outline + introduction  

**Deliverable**:   
- qaplibria/ repo functional  
...
Week 3-4: ATLAS Optimization + FlowLibria  
Week 5-6: Resource Allocation + AllocLibria  
Week 7-8: Integration + MetaLibria  
Week 9-10: Adversarial Testing + DualLibria  
Week 11-12: Evolution + EvoLibria  
...
## Decision Points  
**Week 2 Checkpoint**:   
- Decision: Is QAPLibria showing promising results?  
- If yes: Proceed to FlowLibria  
- If no: Extend QAPLibria investigation, delay schedule  
...
|---|-------------|--------|---------|-------|----------|------------|
| 1 | Contextual QAP | QAPLibria | 🟢 Strong | EJOR | 6mo | High |
| 2 | Confidence-Aware DAG | FlowLibria | 🟢 Strong | AAMAS | 9mo | High |
| 3 | Constrained Thompson | AllocLibria | 🟡 Moderate | NeurIPS WS | 9mo | Medium |
| ... | | | | | | |
...
3. ATLAS production logs (real agent performance data)
...
### OR-Tools (Google)
**Use Cases**: QAPLibria baseline (assignment), FlowLibria (scheduling)
**Integration**:
```python  
from ortools.sat.python import cp_model  

# Example: Assignment problem in QAPLibria  
...
AutoGen (Microsoft)
Use Cases: ATLAS agent orchestration
Integration: [Detailed example]

LangGraph
Use Cases: Workflow graphs in FlowLibria
Integration: [Detailed example]

Integration Matrix
Table
Libria Solver	Primary Tool	Secondary Tools	Custom Implementation
QAPLibria	OR-Tools (baseline)	DEAP (GA), custom tabu	Contextual learning
FlowLibria	LangGraph (workflow)	NetworkX (graphs)	Confidence routing
...			
Installation Guide
...
**Example: `PROMPT_QAPLibria.md`**

```markdown  
# Superprompt: QAPLibria Development  

## Context  
You are implementing QAPLibria, the agent-task assignment optimizer for ATLAS/TURING. You have:  
...
## Your Mission  
Build QAPLibria v1.0 in 3 weeks with these phases:  
...
Goal: Comprehensive experiments + ATLAS integration

Tasks:

ATLAS synthetic benchmark:
...
Package as library: pip install qaplibria
Create adapter for ATLAS agent system
Test in real ATLAS workflow
Documentation:
...
### Decision 1: QAPLibria Cost Predictor Choice  
**Status**: Open  
**Options**:  
...
**Rationale**: Parallel development (ATLAS + Libria), leverage different AI capabilities  
...
## System Architecture
- [ ] ATLAS/TURING definition extracted and clear
...
- [ ] Integration with Optilibria clarified

## Seven Solvers
For EACH solver (QAPLibria, FlowLibria, AllocLibria, GraphLibria, MetaLibria, DualLibria, EvoLibria):
...
QAPLibria ─┐
FlowLibria ─┼─→ MetaLibria (meta-learns over all)
AllocLibria─┘
GraphLibria ─┐
DualLibria ──┼─→ EvoLibria (evolves all architectures)
MetaLibria ──┘

ini

**Check**:
- [ ] MetaLibria correctly depends on others (can't meta-learn before they exist)
- [ ] EvoLibria is last (needs full system to evolve)
- [ ] No circular dependencies

### Timeline Realism
- [ ] QAPLibria (6 months) is achievable given scope
- [ ] Parallel solver development doesn't overload Account 2
- [ ] ATLAS development (Account 1) pace matches Libria integration needs

### Resource Allocation
- [ ] Account 1 budget ($1000/3 days) sufficient for ATLAS?
...
- Could they explain how ATLAS calls QAPLibria? (Yes/No)
- Could they list all 40 agent types? (Yes/No)

### PROMPT_QAPLibria.md
**Question**: Can a developer execute this prompt and build QAPLibria in 3 weeks?
...
**Trigger**: 
- User mentioned "agent-task assignment" for ATLAS
...
### Risk 1: QAPLibria Novelty Insufficient
...
### Risk 2: EvoLibria Too Ambitious
...
### Risk 7: ATLAS and Libria Integration Friction
...
|------|--------|-------------|-------------------|-------|
| QAPLibria Novelty | 🟢 Green | 20% | Yes | Account 2 |
| ... | | | | |
...
**Scope flex**: Can descope lower-priority solvers (GraphLibria, DualLibria) if needed
5.3: Success Metrics
...
- Run each with baseline vs. ATLAS
...
- Baseline (static QAP): Optimality gap = X% (measure on ATLAS tasks)
- QAPLibria (contextual): Target gap = X-10%
...
**Metrics by Month**:
- Month 6: 1 paper submitted (QAPLibria → EJOR)
- Month 9: 2 papers submitted (FlowLibria → AAMAS, AllocLibria → NeurIPS WS)
...
|-----------|--------|--------|-------|
| QAPLibria baseline accuracy | Match QAPLIB within 5% | | |
...
✅ ATLAS system operational and demonstrably better than baseline
...
🌟 Industry adoption (company uses ATLAS/TURING)
...
✅ Understand the complete ATLAS/TURING + Libria Suite architecture in 1 hour of reading
...
3. **Launch Development**:  
   - **Account 1 (ATLAS)**: Use `ARCHITECTURE_MASTER.md` Section 1 + relevant solver specs  
   - **Account 2 (QAPLibria)**: Use `PROMPT_QAPLibria.md` + supporting docs  
